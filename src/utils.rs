...
pub fn random_inc_dec_clamp_signed<R, T>(rng: &mut R, value: T, lower_end: T, upper_end: T, min: T, max: T) -> T
where
    R: Rng,
    T: Copy + Signed + PartialOrd + SampleUniform,
{
    let lower = if value - lower_end < min { min } else { value - lower_end };
    let upper = if value + upper_end > max { max } else { value + upper_end };
...
pub fn random_inc_dec_clamp_unsigned<R, T>(rng: &mut R, value: T, lower_end: T, upper_end: T, min: T, max: T) -> T
where
    R: Rng,
    T: Copy + Unsigned + SaturatingAdd + SaturatingSub + PartialOrd + SampleUniform,
{
    let lower = if value.saturating_sub(&lower_end) < min { min } else { value.saturating_sub(&lower_end) };
